...
import { saveBreathingSession } from "../services/database.ts";
import type { BreathingSession } from "../types/database.ts";
...
export const BreatheSyncPage = ({ onNavigate }: BreatheSyncPageProps) => {
  ...
  const MINIMUM_CYCLES_FOR_COMPLETION = 3; // Require at least 3 cycles to consider it "completed"
  ...
  const stopBreathing = () => {
    ...
    // Save breathing session if it was started
    if (sessionStartTimeRef.current && totalCyclesRef.current >= MINIMUM_CYCLES_FOR_COMPLETION) {
      ...
      const breathingSession: BreathingSession = {
        ...
        cyclesCompleted: totalCyclesRef.current,
        durationSeconds,
        completed: true,
        createdAt: sessionStartTimeRef.current,
      };
      saveBreathingSession(breathingSession).catch((error) => {
        ...
        if (prev <= 1) {
          if (phase === "inhale") {
            ...
            if (pattern.pause > 0) {
              ...
              totalCyclesRef.current = newCount;
              setCycleCount(newCount);
              if (newCount >= MINIMUM_CYCLES_FOR_COMPLETION) {
          ...
          <div className="mt-16 flex flex-col items-center gap-4">
            {!isActive ? (
              ...
              <>
                ...
                {hasCompletedMinimum && cycleCount >= MINIMUM_CYCLES_FOR_COMPLETION && (
                  <button
                    onClick={() => {
                      // Mark that we're returning from exercise
                      sessionStorage.setItem("returnFromExercise", "true");
                      if (onNavigate) {
                        onNavigate("canvas");
                      } else {
                        sessionStorage.setItem("navigateTo", "canvas");
