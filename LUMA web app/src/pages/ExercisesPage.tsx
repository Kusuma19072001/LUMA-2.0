...
import { saveExerciseSession } from "../services/database.ts";
import type { ExerciseSession } from "../types/database.ts";
...
export const ExercisesPage = () => {
  ...
  const startSession = (exercise: Exercise) => {
    ...
    clearActiveInterval();
    setActiveSession({ exercise, remaining: totalSeconds, status: "running", startTime });
        ...
        if (nextRemaining === 0) {
          ...
          if (prev.startTime) {
            ...
            const exerciseSession: ExerciseSession = {
              ...
              exerciseId: prev.exercise.id,
              exerciseName: prev.exercise.title,
              durationMinutes: prev.exercise.minutes,
              durationSeconds,
              completed: true,
              completedAt: Date.now(),
              createdAt: prev.startTime,
            };
            saveExerciseSession(exerciseSession).catch((error) => {
  ...
  const handleFinishEarly = () => {
    ...
    setActiveSession((prev) => {
      ...
      // Save exercise session (completed early)
      if (prev.startTime) {
        const actualDurationSeconds = prev.exercise.minutes * 60 - prev.remaining;
        const exerciseSession: ExerciseSession = {
          id: crypto.randomUUID(),
          exerciseId: prev.exercise.id,
          exerciseName: prev.exercise.title,
