...
import { Sun, Wind, BookOpen, Heart, Sparkles, Music, Moon, Shield, Bell, Circle, Quote, X, LogOut } from "lucide-react";
...
export const ProfilePage = ({ onNavigate: _onNavigate }: ProfilePageProps = {}) => {
  const { user, updateUserStats, logout } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      // Update preferences with user's name
      setPreferences((prev) => ({
        ...prev,
        userName: `${user.firstName} ${user.lastName}`,
      }));
      
      // Update stats with user's data from DB
      setStats((prev) => ({
        ...prev,
        mindfulnessStreak: user.streak || 0,
        completedExercises: user.completedExercises || 0,
        reflectionsCount: user.reflectionsCount || 0,
      }));
    }
    ...
    if (saved) {
      try {
        ...
        setPreferences((_prev) => ({
          ...parsed,
          userName: user ? `${user.firstName} ${user.lastName}` : parsed.userName,
        }));
      } catch (error) {
  ...
  useEffect(() => {
    const loadPerformanceStats = async () => {
      try {
        ...
        allMoodEntries.forEach((entry) => {
          // Only include entries with celebration guidance type and a user-written journal entry
          if (entry.reflection?.guidanceType === "celebration" && entry.journalEntry && entry.journalEntry.trim().length > 0) {
        ...
        if (user) {
          await updateUserStats({
            streak: finalStats.mindfulnessStreak,
            completedExercises: finalStats.completedExercises,
            reflectionsCount: finalStats.journalEntries,
          });
        }
      } catch (error) {
    ...
    // Listen for 'journalSaved' events to refresh reflections
    const handleJournalSaved = () => {
      loadPerformanceStats();
    };
    window.addEventListener('journalSaved', handleJournalSaved);
    return () => {
      window.removeEventListener('journalSaved', handleJournalSaved);
    };
  }, []);
    ...
    <>
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-full border border-emerald-200/60 bg-white/90 px-5 py-2.5 text-sm font-semibold text-emerald-700 shadow-sm backdrop-blur-sm transition-all hover:scale-105 hover:border-emerald-300 hover:bg-emerald-50/80 hover:shadow-md"
              >
            ...
            {isLoadingStats ? (
              ...
              <div className="rounded-2xl bg-emerald-50/50 p-6 text-center">
                <p className="text-emerald-700/70">
                  {positiveReflections.length} {positiveReflections.length === 1 ? "moment" : "moments"} saved
                </p>
                <p className="mt-2 text-sm text-emerald-600/60">
                  Click "see my moments" to view your collection
                </p>
              </div>
            )}
            <button
      ...
      {isMomentsModalOpen && (
        <div
          ...
          onClick={() => setIsMomentsModalOpen(false)}
        >
          <div
            className="relative w-full max-w-2xl max-h-[85vh] rounded-3xl border border-white/60 bg-white/95 p-6 shadow-2xl backdrop-blur overflow-hidden flex flex-col transform transition-all duration-300 scale-100"
          ...
          >
            ...
            <div className="mb-6 pr-8">
              ...
              <p className="mt-2 text-sm text-emerald-700/80">
                Your collection of positive moments and reflections
              </p>
            </div>

            <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar">
              {isLoadingStats ? (
                <p className="text-emerald-700/70 text-center py-8">Loading reflections...</p>
              ) : positiveReflections.length === 0 ? (
                <div className="rounded-2xl bg-emerald-50/50 p-6 text-center">
                  <p className="text-emerald-700/70">No positive reflections yet</p>
                  <p className="mt-2 text-sm text-emerald-600/60">
