...
import { saveMoodEntry, getMoodEntryById, getMoodEntries } from "../services/database.ts";
...
export const MoodCanvasPage = ({ onNavigate }: MoodCanvasPageProps) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // record mood in analytics (non-async, safe)
      try {
        // call and handle rejection so we don't have to make handler async
        recordMood().catch(err => console.warn("recordMood failed", err));
      } catch (err) {
        console.warn("recordMood invocation failed", err);
      }
      ...
      try {
        ...
        setArtwork((prev) => {
          ...
          // Detect positive mood keywords in moodPrompt
          const positiveMoodRegex = /happy|joy|grateful|excited|thankful|love|loving|peaceful|optimistic|content|delighted|uplifted|inspired|good|great|amazing|fantastic|cheerful|hopeful|blessed/i;
          let actualReflection = reflection ? { ...reflection } : prev.reflection ? { ...prev.reflection } : undefined;
          if (actualReflection && !actualReflection.guidanceType) {
            actualReflection.guidanceType = "celebration";
          } else if (actualReflection && actualReflection.guidanceType !== "celebration" && positiveMoodRegex.test(moodPrompt.trim())) {
            // Force guidanceType to 'celebration' if user mood is positive
            actualReflection.guidanceType = "celebration";
          }
  ...
  const handleSaveJournal = async () => {
    ...
    try {
      ...
      if (currentMoodEntryId) {
        entryToUpdate = await getMoodEntryById(currentMoodEntryId);
      }
      // Fallback: find the most recent 'celebration' entry in last 5 minutes if currentMoodEntryId missing
      if (!entryToUpdate) {
        const now = Date.now();
        const entries = await getMoodEntries();
        entryToUpdate = entries.find(
          (e) =>
            e.reflection?.guidanceType === "celebration" &&
            e.createdAt > now - 5 * 60 * 1000
        );
      }
      ...
      if (entryToUpdate) {
        ...
        window.dispatchEvent(new CustomEvent("journalSaved"));
...
// DEBUG: run this in DevTools console to inspect analytics_history count:
// (async () => { const req = indexedDB.open('LumaAnalyticsDB'); req.onsuccess = () => { const db = req.result; const store = db.transaction('analytics_history','readonly').objectStore('analytics_history'); store.getAll().onsuccess = (e) => console.log('analytics_history', e.target.result); }; })();
