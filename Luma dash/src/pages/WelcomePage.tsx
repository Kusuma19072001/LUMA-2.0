...
const quickActions = [
  { label: "Reminders", icon: CalendarClock, tone: "bg-emerald-100 text-emerald-700" },
  { label: "Achievements", icon: Star, tone: "bg-amber-100 text-amber-700" },
];

const actionTiles = [
  { title: "Start Workout", description: "Move with guidance from Clara.", accent: "bg-emerald-50 text-emerald-600" },
  { title: "My Analytics", description: "Track your progress and trends.", accent: "bg-amber-50 text-amber-600" },
  { title: "DearMind", description: "Your private journal", accent: "bg-sky-50 text-sky-600" },
  { title: "More Coming Soon", description: "Stay tuned for new tools.", accent: "bg-rose-50 text-rose-600" },
];

export const WelcomePage = ({ onStartChat }: WelcomePageProps) => {
  return (
    <div className="mx-auto flex max-w-5xl flex-col gap-8 px-4 py-10">
      <section className="space-y-2 text-emerald-900">
        ...
        <h1 className="text-4xl font-semibold">Your calm voice in a noisy world.</h1>
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)]">
        ...
        <div className="rounded-4xl border border-white/60 bg-white/80 p-6 shadow-2xl backdrop-blur">
          ...
          <button className="mt-6 inline-flex items-center justify-center rounded-full bg-amber-400 px-6 py-3 text-sm font-semibold text-amber-900 shadow-lg transition hover:bg-amber-500">
            Let's Go
          </button>
        </div>
      </section>
